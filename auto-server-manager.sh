...
# ============================================
# Auto Server Manager - Keep Dev Server Alive
# ============================================
...
cleanup() {
    ...
    # Kill existing manager
    if [ -f "$MANAGER_PID_FILE" ]; then
        OLD_MANAGER_PID=$(cat "$MANAGER_PID_FILE")
        if ps -p "$OLD_MANAGER_PID" > /dev/null 2>&1; then
            log "Killing existing manager (PID: $OLD_MANAGER_PID)"
            kill "$OLD_MANAGER_PID" 2>/dev/null
            sleep 1
            kill -9 "$OLD_MANAGER_PID" 2>/dev/null
        fi
...
monitor_server() {
    ...
    while true; do
        ...
        log "Check #$CHECK_COUNT - Monitoring server..."
...
log "=========================================="
log "Auto Server Manager Started"
log "=========================================="
log "Manager PID: $$"
